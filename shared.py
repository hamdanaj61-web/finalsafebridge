"""Shared views: vibrant entry screen, role selection cards, sign in, and anonymous reporting."""
import streamlit as st
from ai import analyze_report
from database import create_report, get_setting
from icons import icon_svg, school_mark_svg
from views.guided_report import render_guided_report_wizard

def render_login(login_fn):
    # ── Vibrant Page Header ──
    st.markdown(f"""
    <div class="sb-page-header">
        <div class="sb-pill-badge">
            {icon_svg("shield_check", size=14, color="#4F46E5")}
            <span>School Safeguarding & Wellbeing Platform</span>
        </div>
        <div style="display: flex; align-items: center; gap: 1rem; margin-top: 0.2rem; margin-bottom: 0.5rem;">
            {school_mark_svg(46)}
            <div>
                <h1 style="font-size: 2rem; font-weight: 800; letter-spacing: -0.025em; margin: 0; color: #0F172A;">SafeBridge</h1>
            </div>
        </div>
        <p style="font-size: 1rem; color: #64748B; margin: 0; line-height: 1.5;">
            A safe, private, and human-led space to ask for support, speak up against bullying, or connect with counselors.
        </p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("<p style='font-size: 0.95rem; font-weight: 700; color: #1E293B; margin-bottom: 0.85rem;'>Select an option to continue</p>", unsafe_allow_html=True)

    cols = st.columns(4)

    # 1. Student card
    with cols[0]:
        st.markdown(f"""
        <div class='card'>
            <div class="sb-icon-box">{icon_svg("user", size=24, color="#4F46E5")}</div>
            <h3>Student</h3>
            <p class='muted'>Wellbeing check-ins, guided help, and appointment booking</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Continue as Student", key="role_btn_student", use_container_width=True):
            st.session_state.login_role = "student"

    # 2. Counselor card
    with cols[1]:
        st.markdown(f"""
        <div class='card'>
            <div class="sb-icon-box">{icon_svg("compass", size=24, color="#4F46E5")}</div>
            <h3>Counselor</h3>
            <p class='muted'>Review concerns, triage reports, and coordinate student care</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Counselor Portal", key="role_btn_counselor", use_container_width=True):
            st.session_state.login_role = "counselor"

    # 3. Administrator card
    with cols[2]:
        st.markdown(f"""
        <div class='card'>
            <div class="sb-icon-box">{icon_svg("chart", size=24, color="#4F46E5")}</div>
            <h3>Administrator</h3>
            <p class='muted'>Privacy-preserving analytics, policy settings, and accounts</p>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Admin Portal", key="role_btn_admin", use_container_width=True):
            st.session_state.login_role = "admin"

    # 4. Anonymous Reporting card (Radiant emerald highlight)
    allow_anon = get_setting("allow_anonymous_reports", "1") == "1"
    with cols[3]:
        if allow_anon:
            st.markdown(f"""
            <div class='sb-card-highlight'>
                <div class="sb-icon-box">{icon_svg("anonymous", size=24, color="#10B981")}</div>
                <h3>Anonymous report</h3>
                <p>Speak up safely without signing into an account</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Speak up safely", key="btn_anon_flow", type="primary", use_container_width=True):
                st.session_state.anonymous_mode = True
                st.rerun()
        else:
            st.markdown(f"""
            <div class='card' style="opacity: 0.65;">
                <div class="sb-icon-box" style="background: #F1F5F9; color: #94A3B8;">{icon_svg("anonymous", size=24, color="#94A3B8")}</div>
                <h3>Anonymous report</h3>
                <p class='muted'>Disabled by school administrator</p>
            </div>
            """, unsafe_allow_html=True)

    # ── Role Login Form ──
    if role := st.session_state.get("login_role"):
        st.markdown("<div style='margin-top: 1.5rem;'></div>", unsafe_allow_html=True)
        with st.container(border=True):
            st.subheader(f"{role.title()} sign in")
            with st.form("login_form"):
                username = st.text_input("Username")
                password = st.text_input("Password", type="password")
                submitted = st.form_submit_button(f"Sign in to {role.title()} Account", type="primary", use_container_width=True)
            if submitted:
                if login_fn(username, password, role):
                    st.rerun()
                else:
                    err = st.session_state.get("login_error") or "Those details do not match this account type, or the account is inactive."
                    st.error(err)

def render_anonymous():
    st.markdown(f"""
    <div style="padding: 1.2rem 0 1rem 0; border-bottom: 1px solid var(--sb-border); margin-bottom: 1.5rem;">
        <div style="display: flex; align-items: center; gap: 0.75rem;">
            <div class="sb-icon-box" style="width: 40px; height: 40px; background: #ECFDF5; color: #10B981;">
                {icon_svg("anonymous", size=22, color="#10B981")}
            </div>
            <div>
                <h2 style="font-size: 1.5rem; font-weight: 800; margin: 0; color: #0F172A;">Anonymous report</h2>
                <p style="font-size: 0.92rem; color: #64748B; margin: 0.15rem 0 0 0;">Your submission is completely unlinked from any account or IP address.</p>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    if st.session_state.get("anon_submitted"):
        st.success("Your anonymous report has been sent for counselor review. Thank you for speaking up.")
        if st.button("Back to home", type="secondary"):
            st.session_state.anon_submitted = False
            st.session_state.anonymous_mode = False
            for k in ["anon_guided_step", "anon_guided_data"]:
                st.session_state.pop(k, None)
            st.rerun()
        return

    # Visual separation for consent notice
    st.markdown(f"""
    <div class="sb-notice-block" style="border-left-color: #10B981;">
        <div class="sb-notice-title">
            {icon_svg("info", size=18, color="#10B981")}
            <span>Privacy and reporting notice</span>
        </div>
        <div class="sb-notice-body">
            <strong>What is collected:</strong> Only the details and optional evidence you provide below.<br>
            <strong>Identity:</strong> Fully anonymous. No IP addresses or account identifiers are stored.<br>
            <strong>Reviewers:</strong> Designated school safeguarding counselors only.<br>
            <strong>Advisory tools:</strong> Any automated categorization is an organizational aid only. All safeguarding decisions and outreach are made by human counselors.
        </div>
    </div>
    """, unsafe_allow_html=True)

    st.caption("If someone is in immediate physical danger, please contact local emergency services or alert a trusted adult now.")

    if st.button("← Back to home screen", type="secondary"):
        st.session_state.anonymous_mode = False
        st.rerun()

    # ── Report Submission Mode Selector: Guided Buttons vs Written Form ───
    tab_guided, tab_written = st.tabs([
        "Button-guided report (Interactive & quick)",
        "Standard written form (Type your description)"
    ])

    with tab_guided:
        st.markdown("<p style='font-size: 0.9rem; color: #64748B; margin-bottom: 1rem;'>Select options at each step to build your report with simple clicks.</p>", unsafe_allow_html=True)
        render_guided_report_wizard(user=None, prefix="anon_")

    with tab_written:
        st.markdown("<p style='font-size: 0.9rem; color: #64748B; margin-bottom: 1rem;'>Write about what occurred in your own words.</p>", unsafe_allow_html=True)
        with st.form("anonymous_report"):
            text = st.text_area("Describe what happened", max_chars=6000, placeholder="Share only what you feel safe sharing.")
            evidence = st.file_uploader("Optional evidence", type=["png", "jpg", "jpeg", "pdf"], help="PNG, JPG, or PDF up to 5 MB")
            consent = st.checkbox("I understand how this report will be reviewed and handled", value=True)
            sent = st.form_submit_button("Send report", type="primary")

        if sent:
            if not consent:
                st.error("Please confirm you understand how your report is handled before submitting.")
            elif len(text.strip()) < 10:
                st.error("Please provide a little more detail (at least 10 characters) so a counselor can understand the concern.")
            else:
                try:
                    create_report(None, True, text.strip(), analyze_report(text), evidence)
                    st.session_state.anon_submitted = True
                    st.rerun()
                except ValueError as error:
                    st.error(str(error))
