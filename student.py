...
def home(user):
    ...
    # Flat cards with hairline borders, colored icon chips (no buddy chat)
    cards = [
        ("shield", "blue", "I need help", "Share a concern with a school counselor", "Help"),
        ("smile", "teal", "Weekly check-in", "A brief pulse on how your week is going", "Check-in"),
        ("calendar", "purple", "Book counselor", "Request a private meeting", "Book"),
    ]
    cols = st.columns(3)
    for col, (icon_name, color, title, desc, target) in zip(cols, cards):
        with col:
            st.markdown(f"""
            <div class='card accent-{color}'>
                <div class="sb-icon-chip {color}">{icon_svg(icon_name, size=20)}</div>
                <h3>{title}</h3>
    ...
    cards2 = [
        ("book", "amber", "Learn and stay safe", "Practical tools and guidance for digital safety and respect", "Learn"),
        ("clock", "pink", "My activity", "Review your submitted reports and appointment requests", "My activity")
    ]
    cols2 = st.columns(2)
    for col, (icon_name, color, title, desc, target) in zip(cols2, cards2):
        with col:
            st.markdown(f"""
            <div class='card accent-{color}'>
                <div class="sb-icon-chip {color}">{icon_svg(icon_name, size=20)}</div>
